...
def display_all_pg_details(request,area,rent):
    ...
    print(rent[0],rent[1],rent)
    if rent!=['0'] and area!='area':
        PI=PGInformation.objects.filter((Q(area=area)) & ((Q(fees2__range=(rent[0],rent[1]))) | (Q(fees3_range=(rent[0],rent[1]))) | (Q(fees4_range=(rent[0],rent[1]))) | (Q(fees_range=(rent[0],rent[1]))) ))
    
    elif area=='area':
       PI=PGInformation.objects.filter(  (Q(fees2__range=(rent[0],rent[1]))) | (Q(fees3_range=(rent[0],rent[1]))) | (Q(fees4_range=(rent[0],rent[1]))) | (Q(fees_range=(rent[0],rent[1])))  )  
