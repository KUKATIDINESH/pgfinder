...
def display_alldetails(request,area,rent):
    print(area,rent,type(area),type(rent))
    from django.db.models import Q
    rent=[int(x) for x in rent.split('-')]
    if rent!=['0'] and area!='area':
        PI=PGInformation.objects.filter((Q(area=area)) & ((Q(fees2_eange=(rent[0],rent[1]))) | (Q(fees3_range=(rent[0],rent[1]))) | (Q(fees4_range=(rent[0],rent[1]))) | (Q(fees_range=(rent[0],rent[1]))) ))
    elif area!='area':
       PI=PGInformation.objects.filter(  (Q(fees2_eange=(rent[0],rent[1]))) | (Q(fees3_range=(rent[0],rent[1]))) | (Q(fees4_range=(rent[0],rent[1]))) | (Q(fees_range=(rent[0],rent[1])))  )  
        
    else:
       PI=PGInformation.objects.filter(area=area)
    context={'PI':PI}
    return render(request,'display_all_pg_list',context)
